...
- `GAROON_USERNAME`
- `GAROON_PASSWORD`
- `GAROON_START_DAYS_OFFSET`: 今日から何日前を取得開始日にするか。通常運用は `0` を推奨します
- `GAROON_END_DAYS_OFFSET`: 今日から何日後を取得終了日にするか。通常運用は `31` を推奨します
...
初回 backfill は、過去 1 年から半年先のような broad range を一度だけ同期して `data/sync_state.json` を育てる用途です。通常運用へ切り替えたあとは、`GAROON_START_DAYS_OFFSET=0` と `GAROON_END_DAYS_OFFSET=31` の 1 か月 window を前提に定期実行します。
...
- delete 候補になるのは「その event を最後に確認した fetch window が、今回の fetch window に完全に含まれる」state entry だけです
- broad range backfill で作った event は、より狭い通常運用 window では missing 扱いでも delete されません
- 既存 `sync_state.json` に fetch window metadata が無い古い entry も、安全側に倒して delete しません

このため、broad range で作成した state を残したまま通常運用へ切り替えても、取得範囲外の過去イベントや半年先イベントを通常運用で消さない前提にできます。
...
今回の変更では delete 安全化を優先しており、timer の 15 分化はまだ行っていません。まずは上記の通常運用 window と dry-run で delete 件数を確認してから、必要なら `deploy/systemd/user/garoon-sync.timer` を別途調整してください。
